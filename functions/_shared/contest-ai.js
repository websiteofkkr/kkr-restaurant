...
export async function evaluatePhoto(env, photoUrl) {
  ...
  const food = Math.max(
    0,
    Math.min(25, Number(parsed.food_score) || 0)
  );

  const composition = Math.max(
    0,
    Math.min(20, Number(parsed.composition_score) || 0)
  );

  const atmosphere = Math.max(
    0,
    Math.min(20, Number(parsed.atmosphere_score) || 0)
  );

  const creativity = Math.max(
    0,
    Math.min(20, Number(parsed.creativity_score) || 0)
  );

  const authenticity = Math.max(
    0,
    Math.min(15, Number(parsed.authenticity_score) || 0)
  );
